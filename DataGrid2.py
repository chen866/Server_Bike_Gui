...
class DataGrid2(QWidget):
	...
	def __init__(self,  input_num):
		super().__init__()
		self.setWindowTitle("管理员信息查询，按名查找")
	...
	def getTotalRecordCount(self):			
		self.queryModel.setQuery("select id, name,passwd,telenum,email from admin where name = '%s'"%(self.input_num))
	...
	def recordQuery(self, limitIndex ):	
		szQuery = ("select id, name,passwd,telenum,email from admin  where name='%s' limit %d,%d" % (   self.input_num,  limitIndex , self.PageRecordCount ) )
