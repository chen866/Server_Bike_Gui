...
class addadmin(QWidget, Ui_Form):

    def __init__(self, parent=None):    
        super(addadmin, self).__init__(parent)
    ...
    def ok(self):
        ...
        sql='''insert into admin(name, passwd, telenum, email)   values('%s', '%s', '%s' , '%s') '''%( name,  passwd,  tele,  email)
        print(sql)
        return_num=0
        try:
