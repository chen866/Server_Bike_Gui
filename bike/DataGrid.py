...
class DataGrid(QWidget):
	...
	def __init__(self,  input_num):
		super().__init__()
		self.setWindowTitle("单车信息查询，按号查找")
	...
	def setTableView(self):	
		...
		self.db.setHostName("67.209.xxx.xxxx")
		self.db.setDatabaseName("db_sharedbike")
		self.db.setUserName("root")
		self.db.setPassword("xxxxxxxx")
		...
		self.queryModel.setHeaderData(2,Qt.Horizontal,"经度")
		self.queryModel.setHeaderData(3,Qt.Horizontal,"维度")
		self.queryModel.setHeaderData(4,Qt.Horizontal,"状态")

	# 得到记录数	
	def getTotalRecordCount(self):			
		self.queryModel.setQuery("select id,category,lat,lon,state from bike_info where id = %s"%(self.input_num))
	...
	def recordQuery(self, limitIndex ):	
		szQuery = ("select id,category,lat,lon,state from bike_info where id = %s limit %d,%d" % (   self.input_num,  limitIndex , self.PageRecordCount ) )
