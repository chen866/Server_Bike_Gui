...
class chaneAdmin(QWidget, Ui_Form):
    ...
    def ok(self):
        ...
        db = pymysql.connect(host="67.209.xxxxx.xxxx",user="root",passwd="xxxxxxx",db="db_sharedbike")
        cursor = db.cursor()
        
        sql='''update bike_info set lat=%s, lon=%s, state=%s   WHERE id=%s'''%( passwd,  tele,  email,  id)
        print(sql)
        return_num=0
        try:
