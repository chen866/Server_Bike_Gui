...
class addOrder(QWidget, Ui_Form):
    ...
    def ok(self):
        ...
        db = pymysql.connect(host="67.209.xxx.xxx",user="root",passwd="xxxxxxx",db="db_sharedbike")
        cursor = db.cursor()
        
        sql='''insert into order_info(user_id, bike_id, take_time, return_time)   values(%s, %s, '%s' , '%s') '''%( name,  passwd,  tele,  email)
        print(sql)
        return_num=0
        try:
