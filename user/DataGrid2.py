...
class DataGrid2(QWidget):

	def __init__(self,  input_num):
		super().__init__()
		self.setWindowTitle("用户信息查询，按名查找")
	...
	def setTableView(self):	
		...
		self.db.setPassword("xxxxxxxxxxx")
		...
		self.queryModel.setHeaderData(1,Qt.Horizontal,"用户名")
		...
		self.queryModel.setHeaderData(5,Qt.Horizontal,"钱包金额")
		self.queryModel.setHeaderData(6,Qt.Horizontal,"使用时间")

	# 得到记录数	
	def getTotalRecordCount(self):			
		self.queryModel.setQuery("select * from user_info where username = '%s' "%(self.input_num))
	...
	def recordQuery(self, limitIndex ):	
		szQuery = ("select * from user_info  where username='%s' limit %d,%d" % (   self.input_num,  limitIndex , self.PageRecordCount ) )
