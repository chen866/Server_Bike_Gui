...
from .Ui_deleteAdminByName import *
import pymysql

class DeleteAdminByNum(QWidget, Ui_Dialog):
    
    signal_0=pyqtSignal()
    
    def __init__(self, updateState,  parent=None):    
        super(DeleteAdminByNum, self).__init__(parent)
    ...
    def ok(self):
        ...
        db = pymysql.connect(host="67.209.xxx.xxx",user="root",passwd="xxxxxxxxxx",db="db_sharedbike")
        cursor = db.cursor()
        
        sql="DELETE FROM user_info WHERE username='%s' "%(num)
